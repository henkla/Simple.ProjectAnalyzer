...
[AnalyzerManifest(
    name: nameof(PreReleasePackageReferenceAnalyzer),
    codes:
    [
    ...
    description: "Analyzes project files to detect NuGet package references that use " +
...
public class PreReleasePackageReferenceAnalyzer(IConsoleOutput console) : IAnalyzer
{
    private static readonly Regex PreReleaseRegex = new(@"-\w+", RegexOptions.Compiled);

    public Task Run(Context context)
    {
        console.Verbose($"{nameof(PreReleasePackageReferenceAnalyzer)}.{nameof(Run)} started");
